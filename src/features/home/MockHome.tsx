...
export default function MockHome() {
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      markLoggedIn(); // temporary mock login
    ...
    <main className="fixed inset-0 bg-black text-white font-sans overflow-hidden selection:bg-brand-primary/30">
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-8 relative z-10">
              ...
              >
                ...
                <p className="text-sm font-medium text-white/40 leading-relaxed">
                  The world's first live-first professional commerce platform.
                  Start, scale, and secure your future.
                </p>
              </motion.div>
      ...
      {isLoggedIn && hasCompletedOnboarding && (
        ...
        >
          ...
          <header
            className={`fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 pt-12 pb-6 bg-gradient-to-b from-black/80 via-black/20 to-transparent transition-all duration-500 ${!isNavVisible || activeNav === "profile" ? "opacity-0 -translate-y-full pointer-events-none" : "opacity-100 translate-y-0 pointer-events-auto"}`}
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-2 h-2 rounded-full animate-pulse shadow-glow ${activeNav === "home" ? "bg-brand-primary" : activeNav === "live" ? "bg-red-500" : activeNav === "wallet" ? "bg-emerald-400" : "bg-blue-400"}`}
              />
              <h2
            ...
            <div className="flex items-center gap-4 pointer-events-auto">
              ...
              >
                ...
                <div
                  className={`absolute top-2 right-2 w-2 h-2 rounded-full border-2 border-black ${activeNav === "live" ? "bg-red-500 animate-pulse" : "bg-brand-primary"}`}
                />
              </button>
              <div
                className="w-10 h-10 rounded-full border-2 border-white/20 p-0.5 overflow-hidden active-scale cursor-pointer"
            ...
            {selectedChat && (
              <ChatRoom
                ...
                onOpenBooking={() => {
                  ...
                  const hustler = hustlerId
                    ? MOCK_HUSTLERS.find((h) => h.id === hustlerId)
                    : undefined;
                  if (hustler) {
          ...
          >
            ...
            <div className="absolute inset-0 rounded-[2.5rem] overflow-hidden pointer-events-none">
              <div
                className={`absolute inset-0 opacity-20 transition-colors duration-1000 blur-3xl scale-150 pointer-events-none ${
                  activeNav === "home"
                    ? "bg-brand-primary"
                    : activeNav === "live"
                      ? "bg-red-600"
            ...
            >
              <Home
                ...
                className={activeNav === "home" ? "drop-shadow-glow-red" : ""}
              />
              <span
                className={`text-[8px] uppercase tracking-tighter font-black transition-all duration-300 ${activeNav === "home" ? "opacity-100 translate-y-0" : "opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0"}`}
              >
                Home
              </span>
            </button>
            ...
            >
              <div className="relative">
                <Radio
                  size={22}
                  strokeWidth={activeNav === "live" ? 2.5 : 2}
                  className={
                    activeNav === "live"
                      ? "drop-shadow-glow-red text-red-500"
                      : ""
                  }
                />
                <div
                  className={`absolute -top-1 -right-1 w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse shadow-glow-red ${activeNav === "live" ? "opacity-100" : "opacity-40"}`}
                />
              </div>
              <span
                className={`text-[8px] uppercase tracking-tighter font-black transition-all duration-300 ${activeNav === "live" ? "opacity-100 translate-y-0" : "opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0"}`}
              >
                Live
              </span>
            </button>
            ...
            <div className="relative w-20 flex justify-center -mt-12 mr-[-2px] z-10">
              <div
                className={`absolute inset-0 blur-2xl rounded-full scale-150 animate-pulse opacity-40 transition-colors duration-1000 ${
                  activeNav === "home"
                    ? "bg-brand-primary"
                    : activeNav === "live"
                      ? "bg-red-500"
              ...
              >
                <PlusCircle
                  size={24}
                  className="group-hover:scale-110 transition-transform mb-0.5"
                />
                <motion.span
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
            ...
            >
              <Wallet
                size={22}
                strokeWidth={activeNav === "wallet" ? 2.5 : 2}
                className={
                  activeNav === "wallet"
                    ? "drop-shadow-glow-emerald text-emerald-400"
                    : ""
                }
              />
              <span
                className={`text-[8px] uppercase tracking-tighter font-black transition-all duration-300 ${activeNav === "wallet" ? "opacity-100 translate-y-0" : "opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0"}`}
              >
                Wallet
              </span>
            </button>
            ...
            >
              <User
                size={22}
                strokeWidth={activeNav === "profile" ? 2.5 : 2}
                className={
                  activeNav === "profile"
                    ? "drop-shadow-glow-blue text-blue-400"
                    : ""
                }
              />
              <span
                className={`text-[8px] uppercase tracking-tighter font-black transition-all duration-300 ${activeNav === "profile" ? "opacity-100 translate-y-0" : "opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0"}`}
              >
                Profile
              </span>
            </button>
                  ...
                  {activeNav === "home" && (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-start pr-2">
                        <span className="text-[9px] font-black uppercase tracking-widest text-white group-hover:text-white">
                          Marketing
                        </span>
                        <span className="text-[7px] font-bold uppercase tracking-widest text-white/40 group-hover:text-white/60">
                          Local Stall
                        </span>
                      </div>
                    </motion.button>
              ...
              <ActivityCenter
                ...
                onAction={(action, payload) => {
                  ...
                  if (action === "chat") {
                    setIsChatOpen(true);
                    if (payload?.chatId) {
                      const targetChat = MOCK_CHATS.find(
