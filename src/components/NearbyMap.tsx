...
export default function NearbyMap({
  ...
  const [myStatus, setMyStatus] = useState<"invisible" | "available">(
    "available",
  );
  ...
  const getFilteredHustlers = () => {
    return MOCK_MAP_HUSTLERS.filter((h) => {
      ...
      if (activeFilter === "Available Now" && h.status === "available")
        return true;
    ...
    <div className="fixed inset-0 z-[100] bg-black text-white overflow-hidden flex flex-col font-sans">
        ...
        >
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 flex flex-col items-center">
            <div className="relative">
              <div
                className={`absolute -inset-8 rounded-full border border-white/10 animate-ping opacity-20`}
                style={{ animationDuration: "3s" }}
              />
              <div
                className={`absolute -inset-16 rounded-full border border-white/5 animate-ping opacity-10`}
            ...
            >
              ...
              <div className="absolute bottom-full mb-3 opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-300 w-48 bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl p-3 shadow-2xl flex flex-col gap-2 scale-95 group-hover:scale-100 origin-bottom">
                ...
                <div className="text-[10px] text-white/50">
                  {hustler.category} • {hustler.rating}★
                </div>
                {hustler.isLive && hustler.liveDetails ? (
                  <div className="bg-red-500/10 text-red-500 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded w-fit flex items-center gap-1">
                    ...
                    LIVE NOW • {hustler.liveDetails.viewers}
                  </div>
                ) : (
              ...
              >
                <img
                  src={hustler.avatar}
                  className="w-12 h-12 rounded-full border-[3px] border-black object-cover shadow-2xl"
                  alt={hustler.name}
                />
                <div
                  className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-[3px] border-black ${getStatusColor(hustler.status, hustler.isLive)}`}
                />
              </motion.button>
            </div>
          ))}
          ...
          <div className="absolute top-[20%] left-[80%] z-20 flex flex-col items-center opacity-70">
            ...
            <div className="mt-2 bg-black/60 backdrop-blur-md px-2 py-1 rounded border border-white/10 text-[8px] font-black uppercase text-orange-400">
              Pop-up shop
            </div>
          </div>
        </motion.div>
        ...
        <div className="absolute inset-0 z-10 pt-48 px-6 pb-24 overflow-y-auto bg-black/90 backdrop-blur-3xl">
          <div className="flex flex-col gap-4">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40 mb-2">
              Nearby Activity ({getFilteredHustlers().length})
            </h3>
      ...
      <div className="absolute top-0 left-0 right-0 z-50 pointer-events-none">
        <div className="bg-gradient-to-b from-black/80 via-black/40 to-transparent pt-12 pb-8 px-6 flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between pointer-events-auto">
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-full bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-center hover:bg-white/10 transition-colors"
            >
              <X size={18} />
            </button>
            ...
            <div className="flex items-center bg-black/60 backdrop-blur-md border border-white/10 rounded-full p-1 relative">
              <div
                className={`absolute inset-y-1 w-[calc(50%-4px)] bg-white/10 rounded-full shadow-lg transition-transform duration-300 ${viewMode === "feed" ? "translate-x-[calc(100%+0px)]" : "translate-x-0"}`}
              />
              <button
                onClick={() => setViewMode("map")}
                className={`relative z-10 w-24 h-8 flex items-center justify-center gap-1.5 text-[10px] font-black uppercase tracking-widest transition-colors ${viewMode === "map" ? "text-white" : "text-white/40"}`}
              >
                <MapIcon size={12} /> Map
              </button>
              <button
                onClick={() => setViewMode("feed")}
                className={`relative z-10 w-24 h-8 flex items-center justify-center gap-1.5 text-[10px] font-black uppercase tracking-widest transition-colors ${viewMode === "feed" ? "text-white" : "text-white/40"}`}
              >
                <Grid size={12} /> Feed
              </button>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/80 line-clamp-1">
                    3 hustlers streaming right now
                  </p>
                </div>
              </div>
              <button
        ...
        <div className="flex justify-between items-end pointer-events-auto">
          ...
          <div className="flex flex-col gap-2">
            <button
              onClick={() =>
                setMyStatus((prev) =>
                  prev === "available" ? "invisible" : "available",
                )
              }
      ...
      <AnimatePresence>
        {selectedPin && (
          ...
          >
            <div
              className="w-full bg-[#0c0c0c] border border-white/10 rounded-[2.5rem] p-6 shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Close handle */}
              <div
                className="w-12 h-1.5 bg-white/10 rounded-full mx-auto mb-6 cursor-pointer"
              ...
              <div className="flex gap-5">
                <div className="relative shrink-0">
                  ...
                  />
                  {selectedPin.isLive ? (
                    <div className="absolute -bottom-2 -right-2 bg-red-500 text-white text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded shadow-lg border border-red-400">
                      LIVE
                    </div>
                  ) : (
                    <div
                      className={`absolute -bottom-2 -right-2 text-white text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded shadow-lg border border-white/20 ${getStatusColor(selectedPin.status, false)}`}
                    >
                      {selectedPin.status}
                    </div>
                  )}
              ...
              <div className="mt-6 flex gap-3">
                {selectedPin.isLive ? (
                  ...
                  <button className="flex-1 h-12 bg-white text-black rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl hover:bg-gray-200 transition-colors">
